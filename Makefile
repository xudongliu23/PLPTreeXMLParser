...
SRC_FILES = $(subst xx,pp,$(wildcard $(SRC_DIR)/*.c*))
OBJ_FILES := $(patsubst $(SRC_DIR)/%.cpp, $(OUTPUT_DIR)/%.o, $(SRC_FILES))
LIB_DIR = ./lib
LIB_FILES = -lmystatlib
LIB_SRC_FILES = $(subst xx,pp,$(wildcard $(LIB_DIR)/*.c*))
...
plptreefile=$(shell pwd)/examples/cicp_plptree.xml
...
produce: $(OBJ_FILES)
		$(CXX) -lxerces-c $(OBJ_FILES) -o $(OUTPUT_DIR)/produce
		$(OUTPUT_DIR)/produce $(plptreefile)

$(OUTPUT_DIR)/%.o: $(SRC_DIR)/%.c*
	$(CXX) $(CXXFLAGS) $(XSD_INCLUDE_FLAG) -c $(INCLUDE_FLAGS) $< -o $@
